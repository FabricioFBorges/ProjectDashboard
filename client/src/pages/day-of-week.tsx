...
const average =
  dayData.reduce((acc, d) => acc + d.realized, 0) / dayData.length;
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <div
          className={`text-xs px-2 py-0.5 rounded-full ${isAboveAverage ? "bg-status-positive status-positive" : "bg-status-warning status-warning"}`}
        >
      ...
      <div className="space-y-2">
        <div>
          <p className="text-2xl font-semibold">
            R$ {(data.realized / 1000).toFixed(1)}K
          </p>
        ...
        <div className="grid grid-cols-2 gap-2 pt-2 border-t border-border/50">
          <div>
            <div
              className={`flex items-center gap-1 text-sm font-medium ${vsBudget >= 0 ? "status-positive" : "status-negative"}`}
            >
              {vsBudget >= 0 ? (
          ...
          <div>
            <div
              className={`flex items-center gap-1 text-sm font-medium ${vsLastYear >= 0 ? "status-positive" : "status-negative"}`}
            >
              {vsLastYear >= 0 ? (
                <TrendingUp className="w-3 h-3" />
              ) : (
...
export default function DayOfWeekPage() {
  const [metric, setMetric] = useState<"realized" | "budget" | "lastYear">(
    "realized",
  );
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-xl font-semibold tracking-tight">
                  Análise por{" "}
                  <span className="text-gradient">Dia da Semana</span>
                </h1>
            ...
            <div className="flex items-center gap-3">
              <Select defaultValue="2024-01">
                <SelectTrigger
                  className="w-40 glass-light text-xs"
                ...
                >
                  ...
                  <div className="text-right">
                    <p className="font-mono text-sm">
                      R$ {(day.realized / 1000).toFixed(1)}K
                    </p>
                    <p
                      className={`text-xs ${vsBudget >= 0 ? "status-positive" : "status-negative"}`}
                    >
                      {vsBudget >= 0 ? "+" : ""}
                      {vsBudget.toFixed(1)}% vs Orçado
                    </p>
                  </div>
                </motion.div>
              );
